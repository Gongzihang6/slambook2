...
/**
 * 3D-3D 匹配
 * 从彩色图中提取特征点，计算特征点描述子，匹配两个图像中的特征点，然后计算两个图像中特征点在各自相机坐标系下的3D坐标
 * 然后根据3D-3D匹配计算得到相机 extrinsic
 */
int main(int argc, char **argv){
    ...
    vector<Point3f> pts1, pts2;     // 分别存储相机1和相机2坐标系下匹配的3D点坐标

    // 遍历二维彩色图特征匹配点对
    for (DMatch m : matches){
        // 这里的深度图和彩色图是对齐的，因此可以根据彩色图特征点的像素坐标获取到深度
    ...
    Mat R, t;
    pose_estimation_3d3d(pts1, pts2, R, t);     // ICP
